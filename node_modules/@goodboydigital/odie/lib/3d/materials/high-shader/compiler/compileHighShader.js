...
function compileHooks(programSrc) {
    var _a, _b;
    const parts = {};
    const partMatches = (_b = (_a = programSrc
        .match(findHooksRx)) === null || _a === void 0 ? void 0 : _a.map((hook) => hook.replace(/[{()}]/g, ''))) !== null && _b !== void 0 ? _b : [];
 ...
 */
function addParts(srcParts, parts, name) {
    if (srcParts) {
 ...
 */
function injectFragments(templateSrc, fragmentParts) {
    let out = templateSrc;
 ...
 */
export function compileHighShader(isWebGL2, template, fragments, defines) {
    // TODO make options object - isWebGL2 should be optional
    var _a, _b;
    const cacheId = fragments.map((highFragment) => {
        if (!highFragment.cache)
            highFragment.cache = CACHE_UID++;
    ...
    const vertName = `#define SHADER_NAME ${(_a = template.name) !== null && _a !== void 0 ? _a : 'high'}-vert`;
    const fragName = `#define SHADER_NAME ${(_b = template.name) !== null && _b !== void 0 ? _b : 'high'}-frag`;
