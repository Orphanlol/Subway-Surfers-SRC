...
export function isIPhone6(query) {
    if (!(query.type === 'device' && query.value === 'iPhone6'))
        return false;
    if (Device.ios) {
...
export function isGalaxyS5(query) {
    if (!(query.type === 'device' && query.value === 's5'))
        return false;
    if (Device.android) {
